...
#if defined(NDEBUG)
# error "Worldtrade cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa22365f46e3b204133917b673c09f75cf10c8ffd6e6e2b488dd77fd1c2881c9f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Worldtrade: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Worldtrade Signed Message:\n";
...
{
    // Worldtrade: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Worldtrade: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Worldtrade: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Worldtrade: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Worldtrade: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Worldtrade: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8fcb7305917d3c645aee61eac3cd3489e5c500c2ff4c319a99ea21652c3c9b67");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is worldtrade, Trade For World";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c2eb511549b1005e435ac92d36a21bcc17d4de77d8ac17165abc903c7dcd118d2e0de5d9ac362007de2ae33b768e9cef3815ca8915e5172e937fae18db0b1603") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1541373362;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085939781;

        if (fTestNet)
        {
            block.nTime    = 1541373329;
            block.nNonce   = 385753740;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6719bcfb95c77518295cace8e10104012250401dd2ec25f9cd2ec174e1da50d4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Worldtrade: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WorldtradeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WorldtradeMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WorldtradeMiner(CWallet *pwallet)
{
    printf("WorldtradeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("worldtrade-miner");
    ...
    try { loop {
        ...
        printf("Running WorldtradeMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WorldtradeMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WorldtradeMiner, pwallet));
}
